"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.RestoService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("typeorm");
const RestoMenus_1 = require("../../../entities/RestoMenus");
const typeorm_2 = require("@nestjs/typeorm");
let RestoService = class RestoService {
    constructor(restoMenusRepository) {
        this.restoMenusRepository = restoMenusRepository;
    }
    async findAllMenusResto() {
        return await this.restoMenusRepository.find();
    }
    async findOneMenusResto(reme_Id) {
        const result = await this.restoMenusRepository.findOne({
            where: {
                remeId: reme_Id,
            },
        });
        if (result) {
            return result;
        }
        throw new common_1.HttpException('Categories not found', common_1.HttpStatus.NOT_FOUND);
    }
    async createMenusResto(data) {
        let now = new Date();
        const result = await this.restoMenusRepository.save({
            remeName: data.remeName,
            remeDescription: data.remeDescription,
            remePrice: data.remePrice,
            remeStatus: data.remeStatus,
            remeModifiedDate: now,
        });
        return result;
    }
    async updateMenusResto(id, data) {
        let now = new Date();
        let result = await this.restoMenusRepository.update({
            remeId: id,
        }, {
            remeName: data.remeName,
            remeDescription: data.remeDescription,
            remePrice: data.remePrice,
            remeStatus: data.remeStatus,
            remeModifiedDate: now,
        });
        return result + ' Sukses Mengupdate';
    }
    async deleteMenusResto(id) {
        await this.restoMenusRepository.delete({
            remeId: id,
        });
        return ' Sukses Menghapus';
    }
};
RestoService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_2.InjectRepository)(RestoMenus_1.RestoMenus)),
    __metadata("design:paramtypes", [typeorm_1.Repository])
], RestoService);
exports.RestoService = RestoService;
//# sourceMappingURL=restoMenus.service.js.map